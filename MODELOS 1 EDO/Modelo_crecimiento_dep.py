...
# Definición del modelo de crecimiento dependiente de la densidad
def modelo_crecimiento_densidad(N, t, r, K):
    dN_dt = r * N * ((K - N) / K)
    return dN_dt

# Solución del modelo de crecimiento de población dependiente de densidad usando odeint y RK4
def solve_population_growth(r, K, N0, time, num_points):
    condiciones_iniciales = N0
    t = np.linspace(0, time, num_points)
    sol_exacta = odeint(modelo_crecimiento_densidad, condiciones_iniciales, t, args=(r, K))
    N_exacta = sol_exacta.T[0]

    sol_rk4 = runge_kutta_4(modelo_crecimiento_densidad, condiciones_iniciales, t, r, K)
    N_rk4 = sol_rk4.T[0]
...
def runge_kutta_4(f, y0, t, r, K):
    ...
    for i in range(1, n):
        k1 = h * f(y[i-1], t[i-1], r, K)
        k2 = h * f(y[i-1] + 0.5 * k1, t[i-1] + 0.5 * h, r, K)
        k3 = h * f(y[i-1] + 0.5 * k2, t[i-1] + 0.5 * h, r, K)
        k4 = h * f(y[i-1] + k3, t[i-1] + h, r, K)
...
def plot_phase_plane(r, K, N0, time, num_points):
    condiciones_iniciales = N0
    t = np.linspace(0, time, num_points)
    sol = odeint(modelo_crecimiento_densidad, condiciones_iniciales, t, args=(r, K))
    N = sol.T[0]
    ...
    plt.ylabel('dN/dt')
    plt.title('Plano de fase del modelo de crecimiento dependiente de densidad')
    plt.legend()
    plt.grid(True)
    plt.show()

# Función para ingresar los datos del modelo de crecimiento de población
def input_population_growth_data():
    input_window = Toplevel()
    input_window.title("Ingresar Datos del Modelo de Crecimiento de Población")
    ...
    Label(input_window, text="Capacidad de carga (K):").grid(row=1, column=0, padx=10, pady=5)
    K_entry = Entry(input_window)
    K_entry.grid(row=1, column=1, padx=10, pady=5)

    Label(input_window, text="Tamaño inicial de la población (N0):").grid(row=2, column=0, padx=10, pady=5)
    N0_entry = Entry(input_window)
    N0_entry.grid(row=2, column=1, padx=10, pady=5)

    Label(input_window, text="Tiempo total:").grid(row=3, column=0, padx=10, pady=5)
    time_entry = Entry(input_window)
    time_entry.grid(row=3, column=1, padx=10, pady=5)

    Label(input_window, text="Número de puntos:").grid(row=4, column=0, padx=10, pady=5)
    num_points_entry = Entry(input_window)
    num_points_entry.grid(row=4, column=1, padx=10, pady=5)
    ...
    def on_submit():
        ...
        solve_population_growth(r, K, N0, time, num_points)
    ...
    def on_phase_plane():
        ...
        plot_phase_plane(r, K, N0, time, num_points)
    ...
    phase_plane_button.grid(row=5, column=1, pady=10)
