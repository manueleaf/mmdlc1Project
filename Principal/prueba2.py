        ...
        if selected_option == "Modelo riñon artificial":
            ...
            def on_calculate():
                try:
                    ...
                    tiempo = float(self.entry_tiempo.get())

                    def modelo_exponencial_wrapper(A0, k, tiempo):
                        modelo = ImplementacionModelos()
                        return modelo.modelo_exponencial(A0, k, tiempo)
                    condiciones_iniciales = [0.5, -0.5]
                    t = np.linspace(0,tiempo, 5)
                    soluciones = solve(modelo_exponencial_wrapper, condiciones_iniciales, t, A0, k)
                    df=pd.DataFrame(soluciones)
                    print(df.transpose())
                    self.update_treeview(df.transpose())
                except ValueError:
